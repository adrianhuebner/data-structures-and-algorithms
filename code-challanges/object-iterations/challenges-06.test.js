...
const totalCharacters = (arr) => {
  // total counter
  let totalCount = 0;
  // passing through array and finding each object in array
  for(let i = 0; i < arr.length; i++){
    // within each object if name ++
    if(arr[i].name){
      totalCount ++;
    }
    // within each object if spouse ++
    if(arr[i].spouse){
      totalCount ++;
    }
    // within each object child += children.length
    totalCount += arr[i].children.length;
  }
  return totalCount;
};
