...
const filterStringsWithVowels = (arr) => {
  ...
  let newArr = arr.filter(values => values.match(regEx));
  return (newArr);
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  ...
  let intersection = arr.filter(x => !forbiddenValues.includes(x));
  return (intersection);
};
...
const getStatName = (arr, minBaseStat) => {
  let newArr = [];
  let ans = getBaseStatGreaterThan(arr, minBaseStat);
  ans.forEach(obj => {
    newArr.push(obj.stat.name);
  });
  return newArr;
};
